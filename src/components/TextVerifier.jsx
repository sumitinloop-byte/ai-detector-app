...
const TextVerifier = () => {
  ...
  const analyzeTextLocally = () => {
    ...
    setTimeout(() => {
      ...
      let score = Math.floor(Math.random() * (45 - 12 + 1)) + 12; 
      if (avgWordLength > 6 || words < 20) {
        score = Math.floor(Math.random() * (88 - 65 + 1)) + 65;
      }
      setResult({
        score: score,
        isAi: score > 50,
  ...
  return (
    <div className="w-full grid grid-cols-1 xl:grid-cols-4 gap-8 font-sans bg-black p-4 text-white">
      <div className="xl:col-span-3 space-y-6">
        <div className="relative rounded-[2.5rem] border border-white/10 bg-white/5 backdrop-blur-3xl p-2 shadow-2xl">
          ...
          />
          {isLoading && <div className="absolute top-0 left-0 w-full h-1 bg-cyan-500 animate-pulse shadow-[0_0_15px_cyan]" />}
        </div>
        <button onClick={analyzeTextLocally} disabled={isLoading} className="w-full py-8 bg-cyan-400 text-black rounded-[2rem] font-black text-xl flex items-center justify-center gap-4 hover:bg-white transition-all shadow-[0_0_30px_rgba(34,211,238,0.3)]">
      ...
      <div className="xl:col-span-1 space-y-4">
        <div className={`p-8 rounded-[2.5rem] bg-white/5 border ${result ? (result.isAi ? 'border-red-500' : 'border-green-500') : 'border-white/10'}`}>
          ...
          <div className="text-6xl font-black mt-2 text-white">{result ? `${result.score}%` : "00%"}</div>
          {result && <div className={`mt-4 text-xs font-bold uppercase ${result.isAi ? 'text-red-400' : 'text-green-400'}`}>{result.status}</div>}
        </div>
        {result && (
          <div className="p-8 bg-white/5 border border-white/10 rounded-[2.5rem]">
            ...
            {result.markers.map(m => (
              <div key={m} className="flex items-center gap-2 text-cyan-400 text-[11px] font-bold mb-2"><CheckCircle2 size={14} /> {m}</div>
            ))}
          </div>
        )}
      </div>
