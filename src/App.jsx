...
const PreLoader = () => (
  ...
  >
    <div className="absolute inset-0 bg-[linear-gradient(to_right,#161b22_1px,transparent_1px),linear-gradient(to_bottom,#161b22_1px,transparent_1px)] bg-[size:40px_40px]"></div>
    <div className="relative flex flex-col items-center">
      <motion.div
        animate={{ rotate: 360, scale: [1, 1.1, 1], borderColor: ["#06b6d4", "#ffffff", "#06b6d4"] }}
        transition={{ duration: 3, repeat: Infinity, ease: "linear" }}
        className="w-24 h-24 border-2 border-cyan-500 rounded-full flex items-center justify-center"
      >
        <Cpu className="text-cyan-400 animate-pulse" size={40} />
      </motion.div>
      <div className="mt-10 flex flex-col items-center gap-2">
        <h2 className="text-white font-black tracking-[0.5em] text-xs uppercase text-center">Neural<span className="text-cyan-500">Trace</span> <br/> Forensic Suite</h2>
        <div className="w-48 h-[2px] bg-white/10 rounded-full overflow-hidden mt-4">
          <motion.div initial={{ x: "-100%" }} animate={{ x: "100%" }} transition={{ duration: 2, repeat: Infinity }} className="w-full h-full bg-cyan-500 shadow-[0_0_15px_#06b6d4]" />
        </div>
        <p className="text-[8px] text-cyan-400 font-bold uppercase tracking-[0.2em] mt-2 animate-pulse uppercase">Booting Neural Engines...</p>
      </div>
    </div>
  </motion.div>
);
...
export default function App() {
  ...
  const [appLoading, setAppLoading] = useState(true); // New state
    ...
    <>
      ...
      {!appLoading && (
        <motion.div 
          initial={{ opacity: 0 }} 
          animate={{ opacity: 1 }}
          className="min-h-[100dvh] bg-[#02040a] text-white selection:bg-cyan-500/30 overflow-x-hidden font-sans relative flex flex-col"
        >
          ...
          <nav className="relative z-20 w-full px-6 md:px-12 py-6 border-b border-white/5 backdrop-blur-xl flex justify-between items-center">
            <motion.div className="flex items-center gap-4 cursor-pointer group" whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}>
              <motion.div className="p-2.5 bg-cyan-500 rounded-xl shadow-[0_0_20px_rgba(6,182,212,0.4)] transition-all group-hover:shadow-[0_0_35px_rgba(6,182,212,0.7)]" whileHover={{ rotate: [0, -5, 5, 0] }}>
                <Cpu size={22} className="text-black" />
              </motion.div>
              <div className="flex flex-col">
                <h3 className="text-sm font-black tracking-tighter leading-none uppercase group-hover:text-cyan-400 transition-colors">KIIT UNIVERSITY</h3>
                <p className="text-[9px] font-bold text-cyan-400 tracking-[0.2em] uppercase mt-1 opacity-80 group-hover:opacity-100">Konverge 2026 // Auth-Protocol</p>
              </div>
            </motion.div>
          ...
          <footer className="relative z-20 w-full px-8 py-6 border-t border-white/5 backdrop-blur-md flex flex-col md:flex-row items-center justify-between gap-4">
            <div className="text-[9px] font-black tracking-[0.4em] text-slate-500 uppercase"> Digital Trust Architecture // KIIT HACKATHON 2026 </div>
